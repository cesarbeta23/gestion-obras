// Pase para saltar de Gestión de Obras al ERP sin volver a ingresar.
// Recibe la sesión actual de Gestión, la valida y entrega una sesión del ERP
// solo si el usuario tiene rol en el ERP (usuarios.rol_erp).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!user.rol_erp) { res.status(403).json({ error: 'No tienes acceso al ERP' }); return }
    ...
    const { pin: _p, rol: rolObras, rol_erp, ...resto } = user
    res.status(200).json({ token: pase, exp, user: { ...resto, rol: rol_erp, rol_obras: rolObras } })
  } catch (e) {
