    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #ddd", minWidth: 340, maxWidth: wide ? 720 : 560, width: "94%", maxHeight: "88vh", overflowY: "auto", padding: "1.5rem", boxSizing: "border-box", position: "relative" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 24, color: "#555", lineHeight: 1 }}>×</button>
        </div>
...
function Input({ label, ...props }) {
  return (
    <div style={{ marginBottom: 14 }}>
      {label && <label style={{ fontSize: 13, color: "#555", display: "block", marginBottom: 4 }}>{label}</label>}
      <input style={{ width: "100%", boxSizing: "border-box", padding: "8px 10px", border: "1px solid #ddd", borderRadius: 8, fontSize: 14 }} {...props} />
    </div>
  );
}
...
function Select({ label, children, ...props }) {
  return (
    <div style={{ marginBottom: 14 }}>
      {label && <label style={{ fontSize: 13, color: "#555", display: "block", marginBottom: 4 }}>{label}</label>}
      <select style={{ width: "100%", boxSizing: "border-box", padding: "8px 10px", border: "1px solid #ddd", borderRadius: 8, fontSize: 14 }} {...props}>{children}</select>
    </div>
  );
}
...
function Btn({ children, onClick, variant = "default", disabled, style = {} }) {
  const s = {
    default: { background: "#f5f5f5", border: "1px solid #ddd", color: "#333" },
    primary: { background: "#534AB7", border: "1px solid #534AB7", color: "#fff" },
    danger: { background: "#FCEBEB", border: "1px solid #F09595", color: "#A32D2D" },
    success: { background: "#EAF3DE", border: "1px solid #97C459", color: "#3B6D11" },
    amber: { background: "#FAEEDA", border: "1px solid #EF9F27", color: "#854F0B" },
  };
  return (
...
function Notif({ notifs, setNotifs }) {
  ...
  return (
    <div style={{ position: "fixed", top: 16, right: 16, zIndex: 99999, display: "flex", flexDirection: "column", gap: 8, maxWidth: 320 }}>
      {notifs.map(n => (
        <div key={n.id} style={{ background: n.tipo === "success" ? "#EAF3DE" : "#E6F1FB", border: `1px solid ${n.tipo === "success" ? "#97C459" : "#85B7EB"}`, borderRadius: 10, padding: "12px 16px", display: "flex", gap: 10, alignItems: "flex-start" }}>
          <span style={{ fontSize: 16 }}>{n.tipo === "success" ? "✓" : "🔔"}</span>
          <div style={{ flex: 1, fontSize: 13, color: n.tipo === "success" ? "#3B6D11" : "#185FA5" }}>{n.msg}</div>
          <button onClick={() => setNotifs(ns => ns.filter(x => x.id !== n.id))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, lineHeight: 1 }}>×</button>
        </div>
      ))}
    </div>
  );
...
export default function App() {
  ...
  const sharedProps = { obras, setObras, elementos, setElementos, usuarios, setUsuarios, openModal, closeModal, modals, pushNotif, user, liquidacionesCerradas, setLiquidacionesCerradas };

  return (
    <div style={{ fontFamily: "system-ui, sans-serif", maxWidth: 920, margin: "0 auto", padding: "1rem" }}>
      <Notif notifs={notifs} setNotifs={setNotifs} />
      <Header user={user} setUser={setUser} view={view} setView={setView} selectedObra={selectedObra}
        setSelectedObra={setSelectedObra} setSelectedPiso={setSelectedPiso} setSelectedApto={setSelectedApto} />
      {view === "obras" && <ObrasView {...sharedProps} calcAvanceObra={calcAvanceObra} setSelectedObra={o => { setSelectedObra(o); setView("obra_detalle"); }} />}
      {view === "obra_detalle" && selectedObra && (
      ...
      {view === "apto_detalle" && selectedApto && selectedObra && (
        <AptoDetalle {...sharedProps} apto={selectedApto} piso={selectedPiso}
          obra={obras.find(o => o.id === selectedObra.id)} calcAvanceApto={calcAvanceApto} />
      )}
      {view === "elementos" && user.rol === ROLES.SUPERADMIN && <ElementosView {...sharedProps} />}
      {view === "liquidacion" && <LiquidacionView {...sharedProps} calcLiquidacion={calcLiquidacion} />}
      ...
      <div style={{ display: "flex", gap: 8, borderBottom: "1px solid #eee", paddingBottom: 12 }}>
        {nav.filter(n => n.roles.includes(user.rol)).map(n => (
          <button key={n.key} onClick={() => setView(n.key)}
            style={{ background: view === n.key ? "#EEEDFE" : "transparent", color: view === n.key ? "#534AB7" : "#777", border: view === n.key ? "1px solid #AFA9EC" : "1px solid transparent", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 14, fontWeight: view === n.key ? 500 : 400 }}>
            {n.label}
          </button>
        ))}
      </div>
...
function ObrasView({ obras, setObras, user, usuarios, calcAvanceObra, setSelectedObra, openModal, closeModal, modals, pushNotif }) {
  ...
  function crearObra() {
    ...
    setObras(o => [...o, { id: `obra${Date.now()}`, nombre: form.nombre, direccion: form.direccion, pisos, estado: "activa", tipologias: [], instaladoresAutorizados: [], solicitudes: [] }]);
    setForm({ nombre: "", direccion: "", pisos: 1, aptosPorPiso: 1 });
    closeModal("nuevaObra");
  }

  function solicitarAcceso(obraId) {
    ...
    pushNotif("Solicitud de acceso enviada", "success");
  }
    ...
    <div>
      ...
      <div style={{ display: "grid", gap: 12, marginBottom: 24 }}>
        {obrasVisibles.map(obra => {
          ...
          const pendientes = (obra.solicitudes || []).filter(s => s.estado === "pendiente").length;
          return (
            <div key={obra.id} onClick={() => setSelectedObra(obra)}
              style={{ background: "#fff", border: "1px solid #eee", borderRadius: 12, padding: "1rem 1.25rem", cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}
              onMouseEnter={e => e.currentTarget.style.borderColor = "#AFA9EC"}
              onMouseLeave={e => e.currentTarget.style.borderColor = "#eee"}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontWeight: 500, fontSize: 16, marginBottom: 4 }}>{obra.nombre}</div>
                  <div style={{ fontSize: 13, color: "#777" }}>{obra.direccion}</div>
                </div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  {pendientes > 0 && user.rol === ROLES.SUPERADMIN && (
                    <span onClick={e => { e.stopPropagation(); setAccesoModal(obra.id); }}
                      style={{ background: "#FAEEDA", color: "#854F0B", border: "1px solid #EF9F27", borderRadius: 20, padding: "3px 10px", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>
                      {pendientes} solicitud{pendientes > 1 ? "es" : ""}
                    </span>
                  )}
                  <Badge color="green">{obra.estado}</Badge>
                  {user.rol === ROLES.SUPERADMIN && (
                    <button onClick={e => { e.stopPropagation(); setAccesoModal(obra.id); }}
                      style={{ background: "#E6F1FB", border: "1px solid #85B7EB", color: "#185FA5", borderRadius: 8, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
                      👷 Accesos
                    </button>
                  )}
                </div>
              ...
              <div style={{ display: "flex", gap: 20, marginTop: 14, fontSize: 13, alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ color: "#777" }}>Avance</span>
        ...
        <div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {obrasSinAcceso.map(obra => {
              const solicitud = obra.solicitudes?.find(s => s.userId === user.id);
              return (
                <div key={obra.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f9f9f9", border: "1px solid #eee", borderRadius: 10, padding: "12px 16px" }}>
                  <div>
                    <div style={{ fontWeight: 500, fontSize: 14 }}>{obra.nombre}</div>
                    <div style={{ fontSize: 13, color: "#777" }}>{obra.direccion}</div>
                  </div>
                  {!solicitud && <Btn onClick={() => solicitarAcceso(obra.id)}>Solicitar acceso</Btn>}
                  {solicitud?.estado === "pendiente" && <Badge color="amber">Solicitud pendiente</Badge>}
                  {solicitud?.estado === "rechazado" && <Badge color="coral">Acceso denegado</Badge>}
                </div>
              );
            })}
          </div>
      ...
      {accesoModal && (
        <Modal title={`Accesos — ${obras.find(o => o.id === accesoModal)?.nombre}`} onClose={() => setAccesoModal(null)} wide>
          {(() => {
            const obra = obras.find(o => o.id === accesoModal) || {};
            const solicitudesPendientes = (obra.solicitudes || []).filter(s => s.estado === "pendiente");
            return (
              <div>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: "grid", gap: 8 }}>
                      {solicitudesPendientes.map(s => {
                        ...
                        return (
                          <div key={s.userId} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "#FAEEDA", border: "1px solid #EF9F27", borderRadius: 10 }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontWeight: 500, fontSize: 14 }}>{inst?.nombre}</div>
                              <div style={{ fontSize: 12, color: "#777" }}>Solicitó el {s.fecha}</div>
                            </div>
                            <Btn variant="success" onClick={() => {
                              setObras(obs => obs.map(o => o.id !== accesoModal ? o : { ...o, solicitudes: (o.solicitudes||[]).map(x => x.userId===s.userId ? {...x, estado:"aprobado"} : x), instaladoresAutorizados: [...new Set([...(o.instaladoresAutorizados||[]), s.userId])] }));
                              pushNotif(`Acceso aprobado para ${inst?.nombre}`, "success");
                            }}>Aprobar</Btn>
                            <Btn variant="danger" onClick={() => {
                              setObras(obs => obs.map(o => o.id !== accesoModal ? o : { ...o, solicitudes: (o.solicitudes||[]).map(x => x.userId===s.userId ? {...x, estado:"rechazado"} : x) }));
                            }}>Rechazar</Btn>
                          </div>
                        );
                      })}
                ...
                <div style={{ display: "grid", gap: 8, maxHeight: 360, overflowY: "auto" }}>
                  {instaladores.map(inst => {
                    const autorizado = (obra.instaladoresAutorizados || []).includes(inst.id);
                    return (
                      <div key={inst.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: autorizado ? "#EAF3DE" : "#f9f9f9", border: `1px solid ${autorizado ? "#97C459" : "#eee"}`, borderRadius: 10 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 500, fontSize: 14 }}>{inst.nombre}</div>
                          <div style={{ fontSize: 12, color: "#777" }}>C.C. {inst.cedula || "—"}</div>
                        </div>
                        <button onClick={() => setObras(obs => obs.map(o => {
                          if (o.id !== accesoModal) return o;
                        ...
                        }))} style={{ background: autorizado ? "#FCEBEB" : "#EAF3DE", border: `1px solid ${autorizado ? "#F09595" : "#97C459"}`, color: autorizado ? "#A32D2D" : "#3B6D11", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
                  <Btn onClick={() => setAccesoModal(null)}>Cerrar</Btn>
                </div>
              </div>
            );
          })()}
...
function ObraDetalle({ obra, obras, setObras, user, calcAvanceApto, elementos, usuarios, setSelectedApto, openModal, closeModal, modals, pushNotif }) {
  ...
  function guardarTip() {
    ...
    if (editTip) {
      ...
      setObras(obs => obs.map(o => {
        ...
        return { ...o, tipologias: updated, pisos: o.pisos.map(p => ({ ...p, aptos: p.aptos.map(a => {
          if (a.tipologia !== editTip) return a;
          const nuevosEls = tipForm.elementoIds.map(eid => a.elementos?.find(e => e.elementoId === eid) || { elementoId: eid, completado: false, instaladorId: null, fecha: null, cantidad: 1 });
    ...
    closeModal("tipModal"); setEditTip(null);
  }
  ...
  function asignarTipologia(pisoId, aptoId, tipId) {
    ...
    setObras(obs => obs.map(o => o.id !== obra.id ? o : { ...o, pisos: o.pisos.map(p => p.id !== pisoId ? p : { ...p, aptos: p.aptos.map(a => a.id !== aptoId ? a : { ...a, tipologia: tipId, elementos: nuevosEls }) }) }));
    setAsignando(null);
  }
  ...
  function replicarEnSerie() {
    ...
    setObras(obs => obs.map(o => {
      if (o.id !== obra.id) return o;
      return { ...o, pisos: o.pisos.map(p => ({ ...p, aptos: p.aptos.map(a => {
        const regla = replicaSel.reglas.find(r => r.sufijo === String(a.numero) && r.tipId);
        if (!regla) return a;
        const tip = tipologias.find(t => t.id === regla.tipId);
        if (!tip) return a;
        const nuevosEls = tip.elementoIds.map(eid => a.elementos?.find(e => e.elementoId === eid) || { elementoId: eid, completado: false, instaladorId: null, fecha: null, cantidad: 1 });
        count++;
  ...
  return (
    <div>
      ...
      {tipologias.length > 0 && (
        <div style={{ marginBottom: 18, padding: "12px 16px", background: "#f9f9f9", borderRadius: 10 }}>
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 8 }}>Tipologías</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {tipologias.map(t => (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "#EEEDFE", border: "1px solid #AFA9EC", borderRadius: 20, padding: "4px 12px" }}>
      ...
      {currentObra.pisos?.map(piso => (
        <div key={piso.id} style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 500, color: "#777", marginBottom: 10, borderBottom: "1px solid #eee", paddingBottom: 8 }}>Piso {piso.numero}</div>
            ...
            {piso.aptos?.map(apto => {
              ...
              return (
                <div key={apto.id} onClick={() => apto.tipologia ? setSelectedApto(apto, piso) : null}
                  style={{ background: "#fff", border: "1px solid #eee", borderRadius: 10, padding: 12, cursor: apto.tipologia ? "pointer" : "default" }}
                  ...
                  {tip ? (
                    <>
                      ...
                      <div style={{ height: 4, background: "#f0f0f0", borderRadius: 4, overflow: "hidden", marginBottom: 4 }}>
                    ...
                    asignando === apto.id ? (
                      ...
                      <button onClick={e => { e.stopPropagation(); setAsignando(apto.id); }}
                        style={{ fontSize: 11, color: "#534AB7", background: "#EEEDFE", border: "1px solid #AFA9EC", borderRadius: 6, padding: "3px 6px", cursor: "pointer", marginTop: 4 }}>
                        + tipología
                      </button>
                    )
                ...
                {solicitudesPendientes.length > 0 && (
                  <div style={{ marginBottom: 20 }}>
                    ...
                    {solicitudesPendientes.map(s => {
                      const inst = usuarios.find(u => u.id === s.userId);
                      return (
                        <div key={s.userId} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "#FAEEDA", border: "1px solid #EF9F27", borderRadius: 10, marginBottom: 8 }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 500, fontSize: 14 }}>{inst?.nombre}</div>
                            <div style={{ fontSize: 12, color: "#777" }}>Solicitó el {s.fecha}</div>
                          </div>
                          <Btn variant="success" onClick={() => { setObras(obs => obs.map(o => o.id !== obra.id ? o : { ...o, solicitudes: (o.solicitudes||[]).map(x => x.userId===s.userId ? {...x, estado:"aprobado"} : x), instaladoresAutorizados: [...new Set([...(o.instaladoresAutorizados||[]), s.userId])] })); pushNotif(`Acceso aprobado para ${inst?.nombre}`, "success"); }}>Aprobar</Btn>
                          <Btn variant="danger" onClick={() => setObras(obs => obs.map(o => o.id !== obra.id ? o : { ...o, solicitudes: (o.solicitudes||[]).map(x => x.userId===s.userId ? {...x, estado:"rechazado"} : x) }))}>Rechazar</Btn>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {instaladores.map(inst => {
                    ...
                    return (
                      <div key={inst.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: autorizado ? "#EAF3DE" : "#f9f9f9", border: `1px solid ${autorizado ? "#97C459" : "#eee"}`, borderRadius: 10 }}>
                        ...
                        <button onClick={() => setObras(obs => obs.map(o => { if (o.id !== obra.id) return o; const aut = o.instaladoresAutorizados || []; return { ...o, instaladoresAutorizados: aut.includes(inst.id) ? aut.filter(id => id !== inst.id) : [...aut, inst.id] }; }))}
                          style={{ background: autorizado ? "#FCEBEB" : "#EAF3DE", border: `1px solid ${autorizado ? "#F09595" : "#97C459"}`, color: autorizado ? "#A32D2D" : "#3B6D11", borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
                          {autorizado ? "Revocar" : "Dar acceso"}
                        </button>
                      </div>
        ...
        <Modal title={editTip ? "Editar tipología" : "Nueva tipología"} onClose={() => closeModal("tipModal")}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ maxHeight: 260, overflowY: "auto", border: "1px solid #ddd", borderRadius: 8, padding: 8, background: "#fff" }}>
              {elementos.map(el => (
                <label key={el.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 8px", cursor: "pointer", fontSize: 14, borderRadius: 6, background: tipForm.elementoIds.includes(el.id) ? "#f0effe" : "transparent" }}>
      ...
      {replicaModal && (
        <Modal title="Replicar tipologías por número de apartamento" onClose={() => setReplicaModal(false)} wide>
          <p style={{ fontSize: 13, color: "#777", margin: "0 0 16px" }}>Asigna una tipología a cada número de apartamento. Se aplicará en todos los pisos automáticamente.</p>
          <div style={{ display: "grid", gap: 10, marginBottom: 16 }}>
            {numerosApto.map(sufijo => {
              ...
              return (
                <div key={sufijo} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: tipId ? "#EEEDFE" : "#f9f9f9", border: `1px solid ${tipId ? "#AFA9EC" : "#eee"}`, borderRadius: 10 }}>
                  <div style={{ minWidth: 80 }}>
                    <div style={{ fontWeight: 500, fontSize: 14, color: tipId ? "#534AB7" : "#111" }}>Apto ×{sufijo}</div>
                    <div style={{ fontSize: 12, color: "#777" }}>{cantidad} apto(s)</div>
                  </div>
                  <select style={{ flex: 1, padding: "6px 8px", border: "1px solid #ddd", borderRadius: 8, fontSize: 14 }} value={tipId}
                    onChange={e => { const val = e.target.value; setReplicaSel(r => { const nuevas = r.reglas.filter(x => x.sufijo !== sufijo); if (val) nuevas.push({ sufijo, tipId: val }); return { reglas: nuevas }; }); }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <Btn variant="primary" disabled={!replicaSel.reglas.filter(r => r.tipId).length} onClick={replicarEnSerie}>Aplicar en toda la obra</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function AptoDetalle({ apto, piso, obra, obras, setObras, user, elementos, usuarios, calcAvanceApto, pushNotif }) {
  ...
  function togglePendiente(idx) {
    ...
    setPendientes(p => { const copy = { ...p }; if (copy[idx] !== undefined) delete copy[idx]; else copy[idx] = true; return copy; });
  }
  ...
  function desmarcarElemento(idx) {
    setObras(obs => obs.map(o => { if (o.id !== obra.id) return o; return { ...o, pisos: o.pisos.map(p => { if (p.id !== piso.id) return p; return { ...p, aptos: p.aptos.map(a => { if (a.id !== apto.id) return a; return { ...a, elementos: a.elementos.map((el, i) => i !== idx ? el : { ...el, completado: false, instaladorId: null, fecha: null }) }; }) }; }) }; }));
    pushNotif("Elemento desmarcado", "success");
  }

  const canEdit = user.rol === ROLES.SUPERADMIN || user.rol === ROLES.SUPERVISOR;
  const totalLiquidado = currentApto.elementos?.filter(e => e.completado).reduce((s, el) => { const elem = elementos.find(x => x.id === el.elementoId); return s + (elem?.precio || 0) * (el.cantidad || 1); }, 0) || 0;
  const totalPendiente = Object.keys(pendientes).reduce((s, idx) => { const el = currentApto.elementos[parseInt(idx)]; const elem = elementos.find(x => x.id === el?.elementoId); const cant = cantidades[idx] ?? el?.cantidad ?? 1; return s + (elem?.precio || 0) * cant; }, 0);

  return (
    <div>
      <div style={{ marginBottom: 18 }}>
        <h2 style={{ margin: 0, fontSize: 18, fontWeight: 500 }}>Apto {piso.numero}{String(apto.numero).padStart(2, "0")} — {tip?.nombre || "Sin tipología"}</h2>
        <p style={{ margin: "4px 0 0", fontSize: 13, color: "#777" }}>{obra.nombre} · Piso {piso.numero}</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[["Avance", `${av}%`], ["Instalados", `${currentApto.elementos?.filter(e => e.completado).length || 0}/${currentApto.elementos?.length || 0}`], [user.rol === ROLES.INSTALADOR ? "Mi liquidación" : "Liquidación apto", fmt(totalLiquidado)]].map(([l, v]) => (
          <div key={l} style={{ background: "#f9f9f9", borderRadius: 8, padding: "12px 14px" }}>
            <div style={{ fontSize: 12, color: "#777", marginBottom: 4 }}>{l}</div>
            <div style={{ fontSize: 16, fontWeight: 500 }}>{v}</div>
          </div>
        ))}
      </div>
      {canEdit && <div style={{ marginBottom: 14, padding: "10px 14px", background: "#FAEEDA", border: "1px solid #EF9F27", borderRadius: 10, fontSize: 13, color: "#854F0B" }}>Como {user.rol}, puedes desmarcar elementos instalados con el botón <strong>✕</strong> para corregirlos.</div>}
      {user.rol === ROLES.INSTALADOR && <div style={{ marginBottom: 14, padding: "10px 14px", background: "#EEEDFE", border: "1px solid #AFA9EC", borderRadius: 10, fontSize: 13, color: "#534AB7" }}>Selecciona los elementos que terminaste y presiona <strong>Guardar</strong>.{hayPendientes && <span style={{ marginLeft: 8, fontWeight: 500 }}>· {Object.keys(pendientes).length} por guardar · +{fmt(totalPendiente)}</span>}</div>}
      <div style={{ display: "grid", gap: 8, marginBottom: 20 }}>
        {currentApto.elementos?.map((el, idx) => {
          ...
          return (
            <div key={idx} onClick={() => canToggle && togglePendiente(idx)}
              style={{ display: "flex", alignItems: "center", gap: 12, background: el.completado ? "#EAF3DE" : esPendiente ? "#EEEDFE" : "#fff", border: `1.5px solid ${el.completado ? "#97C459" : esPendiente ? "#AFA9EC" : "#eee"}`, borderRadius: 10, padding: "12px 14px", cursor: canToggle ? "pointer" : "default" }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, border: `2px solid ${el.completado ? "#639922" : esPendiente ? "#534AB7" : "#ccc"}`, background: el.completado ? "#639922" : esPendiente ? "#534AB7" : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {marcado && <span style={{ color: "#fff", fontSize: 16, fontWeight: 700 }}>✓</span>}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 500, fontSize: 14, color: el.completado ? "#27500A" : esPendiente ? "#3C3489" : "#111" }}>{elem?.nombre || el.elementoId}</div>
                {el.completado && instalador && <div style={{ fontSize: 12, color: "#3B6D11" }}>{instalador.nombre} · {el.fecha}</div>}
              ...
              {(elem?.unidad === "ml" || elem?.unidad === "m2") && (
                <div onClick={e => e.stopPropagation()} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <input type="number" min="0.1" step="0.1" value={cantActual} disabled={el.completado && user.rol === ROLES.INSTALADOR}
                    onChange={e => setCantidades(c => ({ ...c, [idx]: Number(e.target.value) }))}
                    style={{ width: 64, textAlign: "center", fontSize: 13, padding: "4px", border: "1px solid #ddd", borderRadius: 6 }} />
                </div>
              )}
              ...
              {canEdit && el.completado && (
                <button onClick={e => { e.stopPropagation(); desmarcarElemento(idx); }}
                  style={{ marginLeft: 4, width: 28, height: 28, borderRadius: 6, border: "1px solid #F09595", background: "#FCEBEB", color: "#A32D2D", cursor: "pointer", fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✕</button>
              )}
            </div>
          );
        })}
      ...
      {user.rol === ROLES.INSTALADOR && (
        <div style={{ position: "sticky", bottom: 0, background: "#fff", borderTop: "1px solid #eee", padding: "14px 0 4px", display: "flex", justifyContent: "flex-end", gap: 10 }}>
          {hayPendientes && <span style={{ fontSize: 14, color: "#777", alignSelf: "center" }}>{Object.keys(pendientes).length} elemento(s) listos</span>}
          <Btn variant="primary" disabled={!hayPendientes} onClick={guardarCambios} style={{ padding: "10px 28px", fontSize: 15 }}>Guardar instalados</Btn>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: 6 }}>
        {elementos.map(el => (
          <div key={el.id} style={{ display: "flex", alignItems: "center", gap: 12, background: "#fff", border: "1px solid #eee", borderRadius: 10, padding: "10px 14px" }}>
...
function LiquidacionView({ obras, elementos, usuarios, user, calcLiquidacion, liquidacionesCerradas, setLiquidacionesCerradas }) {
  const cortes = getCorteFechas();
  const [corteIdx, setCorteIdx] = useState(0);
  const [exportModal, setExportModal] = useState(null);
  const [verHistorial, setVerHistorial] = useState(false);
  const [ajustesModal, setAjustesModal] = useState(null);
  ...
  function getAjuste(instId) { return ajustes[`${corte.label}-${instId}`] || { pasajes: 0, bonificacion: 0 }; }
  function setAjuste(instId, campo, val) { const key = `${corte.label}-${instId}`; setAjustes(a => ({ ...a, [key]: { ...getAjuste(instId), [campo]: Number(val) } })); }

  function calcResumen(instId) {
    const bruto = calcLiquidacion(instId, corte.desde, corte.hasta);
    const retencion = Math.round(bruto * 0.10);
    const subtotal = bruto - retencion;
    const aj = getAjuste(instId);
    return { bruto, retencion, subtotal, pasajes: aj.pasajes || 0, bonificacion: aj.bonificacion || 0, total: subtotal + (aj.pasajes || 0) + (aj.bonificacion || 0) };
  }
  ...
  function cerrarLiquidacion(inst) {
    const resumen = calcResumen(inst.id);
    const rows = detalleInstalador(inst.id, corte.desde, corte.hasta);
    setLiquidacionesCerradas(lcs => [...lcs, { id: `liq-${Date.now()}-${inst.id}`, instId: inst.id, instNombre: inst.nombre, instCedula: inst.cedula, instTelefono: inst.telefono, instBanco: inst.banco, instCuenta: inst.cuenta, corte: corte.label, fechaCierre: new Date().toLocaleDateString("es-CO"), cerradoPor: user.nombre, rows, ...resumen, estado: "pagado" }]);
  }

  function yaCerrada(instId) { return liquidacionesCerradas.some(l => l.instId === instId && l.corte === corte.label); }
  ...
  function exportarExcel(inst, rows, resumen) {
    const lines = [`Liquidación — ${inst.nombre} (C.C. ${inst.cedula}) — Corte: ${corte.label}`, `Tel: ${inst.telefono || "-"} | Banco: ${inst.banco || "-"} | Cta: ${inst.cuenta || "-"}`, "", ["Obra","Apto","Elemento","Cant.","P. unitario","Total","Fecha"].join("\t"), ...rows.map(r => [r.obra, r.apto, r.elemento, r.cantidad, r.precio, r.precio * r.cantidad, r.fecha].join("\t")), "", ["Total bruto","","","","",resumen.bruto,""].join("\t"), ["Retención 10%","","","","",-resumen.retencion,""].join("\t"), ["Subtotal","","","","",resumen.subtotal,""].join("\t"), ["Pasajes","","","","",resumen.pasajes,""].join("\t"), ["Bonificación","","","","",resumen.bonificacion,""].join("\t"), ["TOTAL A PAGAR","","","","",resumen.total,""].join("\t")].join("\n");
  ...
  return (
    <div>
      {exportModal && (
        <Modal title={exportModal.tipo === "pdf" ? "Reporte — Imprimir" : "Reporte Excel — Copiar"} onClose={() => setExportModal(null)} wide>
          {exportModal.tipo === "pdf" ? (
            <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 20, fontSize: 13, lineHeight: 1.7 }}>
              ...
              <div style={{ background: "#f9f9f9", borderRadius: 8, padding: "12px 16px", fontSize: 13 }}>
                {[["Total bruto instalado", exportModal.resumen.bruto], ["Retención 10%", -exportModal.resumen.retencion], ["Subtotal", exportModal.resumen.subtotal], exportModal.resumen.pasajes > 0 ? ["Pasajes", exportModal.resumen.pasajes] : null, exportModal.resumen.bonificacion > 0 ? ["Bonificación", exportModal.resumen.bonificacion] : null, ["Total a pagar", exportModal.resumen.total]].filter(Boolean).map(([l, v], i, arr) => (
                  <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: i < arr.length - 1 ? "1px solid #eee" : "none", fontWeight: i === arr.length - 1 ? 500 : 400, fontSize: i === arr.length - 1 ? 15 : 13, color: i === arr.length - 1 ? "#3B6D11" : "#111", marginTop: i === arr.length - 1 ? 6 : 0 }}>
                    <span>{l}</span><span>{v < 0 ? `— ${fmt(Math.abs(v))}` : fmt(v)}</span>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "#777", margin: "8px 0 0" }}>Clic en el recuadro → Ctrl+A → Ctrl+C</p>
            </div>
          )}
      ...
      {ajustesModal && (
        <Modal title={`Ajustes — ${usuarios.find(u => u.id === ajustesModal)?.nombre}`} onClose={() => setAjustesModal(null)}>
          <p style={{ fontSize: 13, color: "#777", margin: "0 0 16px" }}>Estos valores se suman al subtotal después de la retención del 10%.</p>
          <Input label="Pasajes ($)" type="number" min="0" value={getAjuste(ajustesModal).pasajes} onChange={e => setAjuste(ajustesModal, "pasajes", e.target.value)} />
          <Input label="Bonificación ($)" type="number" min="0" value={getAjuste(ajustesModal).bonificacion} onChange={e => setAjuste(ajustesModal, "bonificacion", e.target.value)} />
          <div style={{ display: "flex", justifyContent: "flex-end" }}><Btn onClick={() => setAjustesModal(null)}>Cerrar</Btn></div>
        </Modal>
      )}
      ...
      {verHistorial ? <HistorialLiquidaciones liquidacionesCerradas={liquidacionesCerradas} user={user} usuarios={usuarios} /> : (
        <>
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {cortes.map((c, i) => (
                <button key={i} onClick={() => setCorteIdx(i)}
                  style={{ background: corteIdx === i ? "#EEEDFE" : "#f5f5f5", color: corteIdx === i ? "#534AB7" : "#777", border: `1px solid ${corteIdx === i ? "#AFA9EC" : "#ddd"}`, borderRadius: 20, padding: "5px 14px", cursor: "pointer", fontSize: 13, fontWeight: corteIdx === i ? 500 : 400 }}>
                  {c.label}
                </button>
              ))}
            </div>
          ...
          {instaladores.map(inst => {
            const rows = detalleInstalador(inst.id, corte.desde, corte.hasta);
            const resumen = calcResumen(inst.id);
            const cerrada = yaCerrada(inst.id);
            return (
              <div key={inst.id} style={{ background: "#fff", border: `1px solid ${cerrada ? "#97C459" : "#eee"}`, borderRadius: 12, padding: "1rem 1.25rem", marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, display: "flex", gap: 6 }}>
                      <Badge color="green">Instalador</Badge>
                      {cerrada && <Badge color="green">✓ Cerrada</Badge>}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    ...
                    {!cerrada && user.rol === ROLES.SUPERADMIN && rows.length > 0 && (
                      <button onClick={() => setAjustesModal(inst.id)} style={{ marginTop: 6, fontSize: 12, background: "#FAEEDA", border: "1px solid #EF9F27", color: "#854F0B", borderRadius: 8, padding: "4px 12px", cursor: "pointer", fontWeight: 500 }}>+ Pasajes / Bonificación</button>
                    )}
                  </div>
                </div>
                {rows.length > 0 && (
                  <div style={{ borderTop: "1px solid #eee", paddingTop: 12, marginBottom: 12 }}>
                    {rows.map((r, i) => (
                      <div key={i} style={{ display: "flex", gap: 10, fontSize: 13, padding: "5px 0", borderBottom: "1px solid #f5f5f5", flexWrap: "wrap" }}>
                        <span style={{ color: "#777", minWidth: 80 }}>{r.obra.substring(0, 14)}</span>
                        <span>Apto {r.apto}</span>
                        <span style={{ flex: 1 }}>{r.elemento}</span>
                        <span style={{ color: "#777" }}>{r.cantidad > 1 ? `×${r.cantidad}` : ""}</span>
                        <span style={{ fontWeight: 500, minWidth: 90, textAlign: "right" }}>{fmt(r.precio * r.cantidad)}</span>
                        <span style={{ color: "#777", minWidth: 76, textAlign: "right" }}>{r.fecha}</span>
                      </div>
                    ))}
                  </div>
                )}
                {rows.length > 0 && (
                  <div style={{ background: "#f9f9f9", borderRadius: 8, padding: "10px 14px", fontSize: 13, marginBottom: 12 }}>
                    {[["Total bruto instalado", resumen.bruto], ["Retención 10%", -resumen.retencion], ["Subtotal", resumen.subtotal], resumen.pasajes > 0 ? ["Pasajes", resumen.pasajes] : null, resumen.bonificacion > 0 ? ["Bonificación", resumen.bonificacion] : null].filter(Boolean).map(([l, v]) => (
                      <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: "1px solid #eee" }}>
                        <span style={{ color: "#777" }}>{l}</span><span>{v < 0 ? `— ${fmt(Math.abs(v))}` : fmt(v)}</span>
                      </div>
                    ))}
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0 0", fontWeight: 500, fontSize: 15, color: "#3B6D11" }}>
                      <span>Total a pagar</span><span>{fmt(resumen.total)}</span>
                    </div>
                  </div>
                )}
                {rows.length === 0 && <p style={{ fontSize: 13, color: "#777", margin: "8px 0" }}>Sin instalaciones en este corte.</p>}
                {puedeExportar && rows.length > 0 && (
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
                    ...
                    {!cerrada && user.rol === ROLES.SUPERADMIN && <Btn variant="amber" onClick={() => cerrarLiquidacion(inst)}>✓ Cerrar y aprobar pago</Btn>}
                  </div>
                )}
              </div>
            );
...
function HistorialLiquidaciones({ liquidacionesCerradas, user, usuarios }) {
  const [filtroInst, setFiltroInst] = useState("");
  const [detalle, setDetalle] = useState(null);
  const instaladores = usuarios.filter(u => u.rol === ROLES.INSTALADOR);
  const liqs = liquidacionesCerradas.filter(l => user.rol === ROLES.INSTALADOR ? l.instId === user.id : (!filtroInst || l.instId === filtroInst)).sort((a, b) => b.id.localeCompare(a.id));
  return (
    <div>
      <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 500 }}>Historial de liquidaciones cerradas</h3>
      {user.rol !== ROLES.INSTALADOR && (
        <Select label="Filtrar por instalador" value={filtroInst} onChange={e => setFiltroInst(e.target.value)}>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {liqs.map(l => (
          <div key={l.id} style={{ background: "#fff", border: "1px solid #97C459", borderRadius: 12, padding: "12px 16px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
              <div>
                <div style={{ fontWeight: 500, fontSize: 15 }}>{l.instNombre}</div>
                <div style={{ fontSize: 12, color: "#777" }}>C.C. {l.instCedula} · Corte: {l.corte}</div>
                <div style={{ fontSize: 12, color: "#777" }}>Cerrado el {l.fechaCierre} por {l.cerradoPor}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 12, color: "#777" }}>Total pagado</div>
                <div style={{ fontSize: 18, fontWeight: 500, color: "#3B6D11" }}>{fmt(l.total)}</div>
                <button onClick={() => setDetalle(detalle?.id === l.id ? null : l)} style={{ fontSize: 12, background: "#EEEDFE", border: "1px solid #AFA9EC", color: "#534AB7", borderRadius: 8, padding: "3px 10px", cursor: "pointer", marginTop: 4 }}>
                  {detalle?.id === l.id ? "Ocultar" : "Ver detalle"}
                </button>
              </div>
            </div>
            {detalle?.id === l.id && (
              <div style={{ marginTop: 12, borderTop: "1px solid #eee", paddingTop: 12 }}>
                {l.rows.map((r, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, fontSize: 12, padding: "4px 0", borderBottom: "1px solid #f5f5f5", flexWrap: "wrap" }}>
                    <span style={{ color: "#777", minWidth: 80 }}>{r.obra?.substring(0, 14)}</span>
                    <span>Apto {r.apto}</span>
                    <span style={{ flex: 1 }}>{r.elemento}</span>
                    <span style={{ fontWeight: 500, minWidth: 90, textAlign: "right" }}>{fmt(r.precio * r.cantidad)}</span>
                ...
                <div style={{ marginTop: 10, background: "#f9f9f9", borderRadius: 8, padding: "8px 12px", fontSize: 12 }}>
                  {[["Total bruto", l.bruto], ["Retención 10%", -l.retencion], ["Subtotal", l.subtotal], l.pasajes > 0 ? ["Pasajes", l.pasajes] : null, l.bonificacion > 0 ? ["Bonificación", l.bonificacion] : null, ["Total pagado", l.total]].filter(Boolean).map(([lb, v], i, arr) => (
                    <div key={lb} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", fontWeight: i === arr.length - 1 ? 500 : 400, color: i === arr.length - 1 ? "#3B6D11" : "#111" }}>
                      <span>{lb}</span><span>{v < 0 ? `— ${fmt(Math.abs(v))}` : fmt(v)}</span>
                    </div>
                  ))}
                </div>
              </div>
...
function UsuariosView({ usuarios, setUsuarios, openModal, closeModal, modals }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {usuarios.map(u => (
          <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, background: "#fff", border: "1px solid #eee", borderRadius: 10, padding: "10px 14px" }}>
