...
async function dbUpsert(table, data) {
  await fetch(`${SUPA_URL}/rest/v1/${table}`, { method: "POST", headers: { ...headers, "Prefer": "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify(data) });
}
...
function Badge({ color, children }) {
  const c = { green:{bg:"#EAF3DE",text:"#3B6D11",border:"#97C459"}, amber:{bg:"#FAEEDA",text:"#854F0B",border:"#EF9F27"}, blue:{bg:"#E6F1FB",text:"#185FA5",border:"#85B7EB"}, gray:{bg:"#F1EFE8",text:"#5F5E5A",border:"#B4B2A9"}, coral:{bg:"#FAECE7",text:"#993C1D",border:"#F0997B"}, purple:{bg:"#EEEDFE",text:"#534AB7",border:"#AFA9EC"} }[color] || {bg:"#F1EFE8",text:"#5F5E5A",border:"#B4B2A9"};
  return <span style={{background:c.bg,color:c.text,border:`1px solid ${c.border}`,borderRadius:6,padding:"2px 10px",fontSize:12,fontWeight:500}}>{children}</span>;
}
...
function Modal({ title, onClose, children, wide }) {
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
      <div style={{background:"#fff",borderRadius:16,border:"1px solid #ddd",minWidth:340,maxWidth:wide?720:560,width:"94%",maxHeight:"88vh",overflowY:"auto",padding:"1.5rem",boxSizing:"border-box"}}>
...
function Btn({ children, onClick, variant="default", disabled, style={} }) {
  const s = { default:{background:"#f5f5f5",border:"1px solid #ddd",color:"#333"}, primary:{background:"#534AB7",border:"1px solid #534AB7",color:"#fff"}, danger:{background:"#FCEBEB",border:"1px solid #F09595",color:"#A32D2D"}, success:{background:"#EAF3DE",border:"1px solid #97C459",color:"#3B6D11"}, amber:{background:"#FAEEDA",border:"1px solid #EF9F27",color:"#854F0B"} };
  return <button onClick={onClick} disabled={disabled} style={{...(s[variant]||s.default),borderRadius:8,padding:"8px 16px",cursor:disabled?"not-allowed":"pointer",fontSize:14,fontWeight:500,opacity:disabled?0.5:1,...style}}>{children}</button>;
}
...
export default function App() {
  ...
  function pushNotif(msg, tipo="info") { const id=Date.now(); setNotifs(ns=>[...ns,{id,msg,tipo}]); setTimeout(()=>setNotifs(ns=>ns.filter(x=>x.id!==id)),5000); }

  async function loadAll() {
    setLoading(true);
    try {
      const [u, e, o, l] = await Promise.all([dbGet("usuarios"), dbGet("elementos"), dbGet("obras"), dbGet("liquidaciones")]);
      if (u.length === 0) { await Promise.all(USUARIOS_DEFAULT.map(u => dbUpsert("usuarios", u))); setUsuarios(USUARIOS_DEFAULT); }
      else setUsuarios(u);
      if (e.length === 0) { await Promise.all(ELEMENTOS_DEFAULT.map(el => dbUpsert("elementos", el))); setElementos(ELEMENTOS_DEFAULT); }
      else setElementos(e);
      setObras(o.map(ob => ({ ...ob, tipologias: ob.tipologias||[], pisos: ob.pisos||[], instaladoresAutorizados: ob.instaladores_autorizados||[], solicitudes: ob.solicitudes||[] })));
      setLiquidaciones(l);
    } catch(err) { pushNotif("Error conectando a la base de datos", "error"); }
    setLoading(false);
  }
  ...
  async function saveObra(obra) {
    const row = { id: obra.id, nombre: obra.nombre, direccion: obra.direccion, estado: obra.estado, tipologias: obra.tipologias||[], pisos: obra.pisos||[], instaladores_autorizados: obra.instaladoresAutorizados||[], solicitudes: obra.solicitudes||[] };
    await dbUpsert("obras", row);
  }
  ...
  function login() {
    ...
    if (u) { setUser(u); setLoginError(""); } else setLoginError("Correo o PIN incorrecto");
  }
  ...
  if (loading) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui",flexDirection:"column",gap:16}}><div style={{fontSize:36}}>🏗️</div><p style={{color:"#777"}}>Cargando...</p></div>;
  if (!user) return <LoginScreen loginData={loginData} setLoginData={setLoginData} login={login} error={loginError} />;

  const sharedProps = { obras, setObras, updateObra, saveObra, elementos, setElementos, usuarios, setUsuarios, openModal, closeModal, modals, pushNotif, user, liquidaciones, setLiquidaciones, loadAll };
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:920,margin:"0 auto",padding:"1rem"}}>
      ...
      {view==="obra_detalle" && selectedObra && <ObraDetalle {...sharedProps} obra={obras.find(o=>o.id===selectedObra.id)||selectedObra} calcAvanceApto={calcAvanceApto} setSelectedApto={(a,p)=>{setSelectedApto(a);setSelectedPiso(p);setView("apto_detalle");}} />}
      {view==="apto_detalle" && selectedApto && selectedObra && <AptoDetalle {...sharedProps} apto={selectedApto} piso={selectedPiso} obra={obras.find(o=>o.id===selectedObra.id)} calcAvanceApto={calcAvanceApto} />}
...
function ObrasView({ obras, updateObra, saveObra, user, usuarios, calcAvanceObra, setSelectedObra, openModal, closeModal, modals, pushNotif, setObras }) {
  ...
  async function crearObra() {
    ...
    const pisos = Array.from({length:Number(form.pisos)},(_,pi)=>({id:`p${Date.now()}${pi}`,numero:pi+1,aptos:Array.from({length:Number(form.aptosPorPiso)},(_,ai)=>({id:`a${Date.now()}${pi}${ai}`,numero:ai+1,tipologia:"",elementos:[]}))}));
    const nueva = {id:`obra${Date.now()}`,nombre:form.nombre,direccion:form.direccion,pisos,estado:"activa",tipologias:[],instaladoresAutorizados:[],solicitudes:[]};
    ...
    <div>
      ...
      <div style={{display:"grid",gap:12,marginBottom:24}}>
        {obrasVisibles.map(obra=>{
          ...
          return (
            <div key={obra.id} onClick={()=>setSelectedObra(obra)} style={{background:"#fff",border:"1px solid #eee",borderRadius:12,padding:"1rem 1.25rem",cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#AFA9EC"} onMouseLeave={e=>e.currentTarget.style.borderColor="#eee"}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div><div style={{fontWeight:500,fontSize:16,marginBottom:4}}>{obra.nombre}</div><div style={{fontSize:13,color:"#777"}}>{obra.direccion}</div></div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  {pendientes>0&&user.rol===ROLES.SUPERADMIN&&<span onClick={e=>{e.stopPropagation();setAccesoModal(obra.id);}} style={{background:"#FAEEDA",color:"#854F0B",border:"1px solid #EF9F27",borderRadius:20,padding:"3px 10px",fontSize:12,cursor:"pointer",fontWeight:500}}>{pendientes} solicitud{pendientes>1?"es":""}</span>}
      ...
      {user.rol===ROLES.INSTALADOR&&obrasSinAcceso.length>0&&(
        <div>
          ...
          {obrasSinAcceso.map(obra=>{
            ...
            return (
              <div key={obra.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#f9f9f9",border:"1px solid #eee",borderRadius:10,padding:"12px 16px",marginBottom:8}}>
              ...
              <div>
                  ...
                  {pends.map(s=>{
                    ...
                    return (
                      <div key={s.userId} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"#FAEEDA",border:"1px solid #EF9F27",borderRadius:10,marginBottom:8}}>
                        ...
                        <Btn variant="success" onClick={()=>updateObra(accesoModal,o=>({...o,solicitudes:(o.solicitudes||[]).map(x=>x.userId===s.userId?{...x,estado:"aprobado"}:x),instaladoresAutorizados:[...new Set([...(o.instaladoresAutorizados||[]),s.userId])]}))}>Aprobar</Btn>
                ...
                <div style={{display:"grid",gap:8,maxHeight:360,overflowY:"auto"}}>
                  {instaladores.map(inst=>{
                    ...
                    return (
                      <div key={inst.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:aut?"#EAF3DE":"#f9f9f9",border:`1px solid ${aut?"#97C459":"#eee"}`,borderRadius:10}}>
...
function ObraDetalle({ obra, obras, updateObra, user, calcAvanceApto, elementos, usuarios, setSelectedApto, openModal, closeModal, modals, pushNotif }) {
  ...
  async function guardarTip() {
    ...
    if (editTip) {
      await updateObra(obra.id, o => ({...o, tipologias:(o.tipologias||[]).map(t=>t.id===editTip?{...t,nombre:tipForm.nombre,elementoIds:tipForm.elementoIds}:t), pisos:o.pisos.map(p=>({...p,aptos:p.aptos.map(a=>{if(a.tipologia!==editTip)return a;const nuevosEls=tipForm.elementoIds.map(eid=>a.elementos?.find(e=>e.elementoId===eid)||{elementoId:eid,completado:false,instaladorId:null,fecha:null,cantidad:1});return{...a,elementos:nuevosEls};})})) }));
      pushNotif(`Tipología actualizada`,"success");
    } else {
    ...
    <div>
        ...
        <Modal title={`Accesos — ${currentObra.nombre}`} onClose={()=>setAccesoObraModal(false)} wide>
          {(()=>{
            ...
            return (<div>
              {pends.length>0&&<div style={{marginBottom:20}}>
                <div style={{fontSize:13,fontWeight:500,marginBottom:10,color:"#854F0B"}}>Solicitudes pendientes</div>
                {pends.map(s=>{const inst=usuarios.find(u=>u.id===s.userId);return(
                  <div key={s.userId} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"#FAEEDA",border:"1px solid #EF9F27",borderRadius:10,marginBottom:8}}>
                    <div style={{flex:1}}><div style={{fontWeight:500,fontSize:14}}>{inst?.nombre}</div><div style={{fontSize:12,color:"#777"}}>Solicitó el {s.fecha}</div></div>
      ...
      {replicaModal&&(
        <Modal title="Replicar tipologías por número de apartamento" onClose={()=>setReplicaModal(false)} wide>
          <p style={{fontSize:13,color:"#777",margin:"0 0 16px"}}>Asigna una tipología a cada número. Se aplicará en todos los pisos automáticamente.</p>
          <div style={{display:"grid",gap:10,marginBottom:16}}>
            {numerosApto.map(sufijo=>{
              ...
              return(<div key={sufijo} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:tipId?"#EEEDFE":"#f9f9f9",border:`1px solid ${tipId?"#AFA9EC":"#eee"}`,borderRadius:10}}>
                ...
                <select style={{flex:1,padding:"6px 8px",border:"1px solid #ddd",borderRadius:8,fontSize:14}} value={tipId} onChange={e=>{const val=e.target.value;setReplicaSel(r=>{const nuevas=r.reglas.filter(x=>x.sufijo!==sufijo);if(val)nuevas.push({sufijo,tipId:val});return{reglas:nuevas};});}}>
...
function AptoDetalle({ apto, piso, obra, obras, updateObra, user, elementos, usuarios, calcAvanceApto, pushNotif }) {
  ...
  function togglePendiente(idx){if(user.rol!==ROLES.INSTALADOR)return;const el=currentApto.elementos[idx];if(el.completado)return;setPendientes(p=>{const c={...p};if(c[idx]!==undefined)delete c[idx];else c[idx]=true;return c;});}

  async function guardarCambios() {
    await updateObra(obra.id, o=>({...o,pisos:o.pisos.map(p=>{if(p.id!==piso.id)return p;return{...p,aptos:p.aptos.map(a=>{if(a.id!==apto.id)return a;const newEls=a.elementos.map((el,i)=>{let u={...el};if(cantidades[i]!==undefined)u.cantidad=cantidades[i];if(pendientes[i]){u.completado=true;u.instaladorId=user.id;u.fecha=new Date().toLocaleDateString("es-CO");}return u;});const allDone=newEls.every(e=>e.completado);if(allDone)supervisores.forEach(s=>pushNotif(`🔔 ${s.nombre}: Apto ${piso.numero}${String(a.numero).padStart(2,"0")} completado en ${obra.nombre}`,"info"));return{...a,elementos:newEls};})};})}));
    pushNotif("Elementos guardados","success");
  ...
  return (
    <div>
      ...
      {canEdit&&<div style={{marginBottom:14,padding:"10px 14px",background:"#FAEEDA",border:"1px solid #EF9F27",borderRadius:10,fontSize:13,color:"#854F0B"}}>Como {user.rol}, puedes desmarcar elementos con ✕ para corregirlos.</div>}
      {user.rol===ROLES.INSTALADOR&&<div style={{marginBottom:14,padding:"10px 14px",background:"#EEEDFE",border:"1px solid #AFA9EC",borderRadius:10,fontSize:13,color:"#534AB7"}}>Selecciona los elementos terminados y presiona <strong>Guardar</strong>.{hayPendientes&&<span style={{marginLeft:8,fontWeight:500}}>· {Object.keys(pendientes).length} por guardar · +{fmt(totalPendiente)}</span>}</div>}
...
function ElementosView({ elementos, setElementos, openModal, closeModal, modals, pushNotif }) {
  ...
  async function guardar(){
    ...
    if(editId)setElementos(els=>els.map(e=>e.id===editId?el:e));
    else setElementos(els=>[...els,el]);
...
function LiquidacionView({ obras, elementos, usuarios, user, calcLiquidacion, liquidaciones, setLiquidaciones }) {
  ...
  function calcResumen(instId){const bruto=calcLiquidacion(instId,corte.desde,corte.hasta),retencion=Math.round(bruto*0.10),subtotal=bruto-retencion,aj=getAjuste(instId);return{bruto,retencion,subtotal,pasajes:aj.pasajes||0,bonificacion:aj.bonificacion||0,total:subtotal+(aj.pasajes||0)+(aj.bonificacion||0)};}

  function detalleInstalador(instId,desde,hasta){
    const rows=[];
    obras.forEach(obra=>obra.pisos?.forEach(piso=>piso.aptos?.forEach(apto=>apto.elementos?.forEach(el=>{
      if(el.completado&&el.instaladorId===instId&&fechaDentroCorte(el.fecha,desde,hasta)){const elem=elementos.find(e=>e.id===el.elementoId);rows.push({obra:obra.nombre,apto:`${piso.numero}${String(apto.numero).padStart(2,"0")}`,elemento:elem?.nombre,cantidad:el.cantidad||1,precio:elem?.precio||0,fecha:el.fecha});}
    }))));
    return rows;
  }

  async function cerrarLiquidacion(inst){
    const resumen=calcResumen(inst.id), rows=detalleInstalador(inst.id,corte.desde,corte.hasta);
  ...
  return (
    <div>
      {exportModal&&<Modal title={exportModal.tipo==="pdf"?"Reporte — Imprimir":"Reporte Excel — Copiar"} onClose={()=>setExportModal(null)} wide>
      ...
      {ajustesModal&&<Modal title={`Ajustes — ${usuarios.find(u=>u.id===ajustesModal)?.nombre}`} onClose={()=>setAjustesModal(null)}>
        <p style={{fontSize:13,color:"#777",margin:"0 0 16px"}}>Se suman al subtotal después de la retención del 10%.</p>
      ...
      {verHistorial?<HistorialLiquidaciones liquidaciones={liquidaciones} user={user} usuarios={usuarios}/>:(
        <>
          ...
          {instaladores.map(inst=>{
            const rows=detalleInstalador(inst.id,corte.desde,corte.hasta), resumen=calcResumen(inst.id), cerrada=yaCerrada(inst.id);
            return(<div key={inst.id} style={{background:"#fff",border:`1px solid ${cerrada?"#97C459":"#eee"}`,borderRadius:12,padding:"1rem 1.25rem",marginBottom:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:10}}>
                <div>
                  ...
                  <div style={{marginTop:6,display:"flex",gap:6}}><Badge color="green">Instalador</Badge>{cerrada&&<Badge color="green">✓ Cerrada</Badge>}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  ...
                  {!cerrada&&user.rol===ROLES.SUPERADMIN&&rows.length>0&&<button onClick={()=>setAjustesModal(inst.id)} style={{marginTop:6,fontSize:12,background:"#FAEEDA",border:"1px solid #EF9F27",color:"#854F0B",borderRadius:8,padding:"4px 12px",cursor:"pointer",fontWeight:500}}>+ Pasajes / Bonificación</button>}
                </div>
              </div>
              {rows.length>0&&<div style={{borderTop:"1px solid #eee",paddingTop:12,marginBottom:12}}>
                {rows.map((r,i)=><div key={i} style={{display:"flex",gap:10,fontSize:13,padding:"5px 0",borderBottom:"1px solid #f5f5f5",flexWrap:"wrap"}}>
                  <span style={{color:"#777",minWidth:80}}>{r.obra.substring(0,14)}</span>
                  <span>Apto {r.apto}</span><span style={{flex:1}}>{r.elemento}</span>
...
function HistorialLiquidaciones({ liquidaciones, user, usuarios }) {
  ...
  return (
    <div>
      ...
      {user.rol!==ROLES.INSTALADOR&&<Select label="Filtrar por instalador" value={filtroInst} onChange={e=>setFiltroInst(e.target.value)}><option value="">Todos los instaladores</option>{instaladores.map(i=><option key={i.id} value={i.id}>{i.nombre}</option>)}</Select>}
...
function UsuariosView({ usuarios, setUsuarios, openModal, closeModal, modals }) {
  ...
  async function guardar(){
    ...
    if(editId)setUsuarios(us=>us.map(x=>x.id===editId?u:x));
    else setUsuarios(us=>[...us,u]);
